...
function Stream(s) {
  ...
  s.addListener('data', function (d) {
    ...
    while (self.buf && self.buf.length > 0) {
      ...
      /* bytes_remaining is a single native global, overwritten by the next
       * unpack() anywhere in the process. Snapshot it before anything else
       * can run, so a 'msg' listener that unpacks cannot desync this loop. */
